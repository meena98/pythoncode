...
def test_function_call_with_keyword_arguments():
    ...
    assert [5, 20, 3, 4] == demo(second=20,first=5)
    assert [5, 20, 30, 4] == demo(second=20, third=30,first=5)
    assert [10, 20, 30, 4] == demo(first=10, third=30,second=20)
    assert [10, 20, 30, 4] == demo(10, third=30, second=20)
...
def simple_format(format, *args):
    ...
    list=format.split(" ")
    if '%0' in list:
        list = [w.replace('%0', args[0]) for w in list]
    if '%1' in list:
        list = [w.replace('%1', args[1]) for w in list]
    if '%2' in list:
        if len(args)<2:
            list = [w.replace('%2', args[2]) for w in list]
    s = ' '.join(list)
    return s
    pass
